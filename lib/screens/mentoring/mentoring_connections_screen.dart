...
import 'profile_card_view.dart';

class MentoringConnectionsScreen extends StatefulWidget {
  const MentoringConnectionsScreen({super.key});

  @override
  State<MentoringConnectionsScreen> createState() => _MentoringConnectionsScreenState();
}

class _MentoringConnectionsScreenState extends State<MentoringConnectionsScreen> {
          ...
          children: [
            ...
            const Column(
              children: [
                ...
                SizedBox(height: 12),
                Text(
                  'No mentorship connections match your current preferences.',
                  style: TextStyle(
                    fontSize: 16,
        ...
        child: Column(
          children: [
            ...
            const Column(
              children: [
                Text(
                  'No More Mentorship Connections',
                  style: TextStyle(
                    fontSize: 22,
                    fontWeight: FontWeight.bold,
                    color: Colors.white,
                ...
                SizedBox(height: 12),
                Text(
                  'You\'ve seen all available mentorship connections in your area!',
                  style: TextStyle(
                    fontSize: 16,
                    color: Colors.white70,
                  ),
    ...
    return Column(
      ...
      children: [
        ...
        _buildSuggestionRow(
          ...
          description: 'Consider connecting with mentors/mentees outside your current age range',
        ),
        
        _buildSuggestionRow(
          icon: Icons.people,
          title: 'Broaden Gender Preferences',
          description: 'Set preferences to \'Everyone\' to see more mentorship opportunities',
        ),
        
        _buildSuggestionRow(
          icon: Icons.business,
          title: 'Adjust Industry Filter',
          description: 'Turn off \'Match by Industry\' to see diverse mentorship options',
        ),
        
        _buildSuggestionRow(
          icon: Icons.school,
          title: 'Switch Connection Type',
          description: 'Try switching between Mentor and Mentee preferences',
        ),
        
        _buildSuggestionRow(
          icon: Icons.refresh,
          title: 'Check Back Later',
          description: 'New mentors and mentees join daily - try again tomorrow!',
        ),
      ],
    );
  }
  ...
  Future<void> _fetchCurrentUserPreferences() async {
    ...
    try {
      ...
      try {
        ...
        await _fetchMentorshipConnections(_currentUser!);
      } catch (e) {
  ...
  Future<void> _fetchMentorshipConnections(UserProfile user) async {
    try {
      ...
      // TEMPORARY: Let's try without connection preference filter first
      print('🔍 Testing: Loading all profiles first to see if any exist...');
      // TODO: Re-enable this filter once we confirm profiles are loading
      // final targetRole = user.connectionPreference == 'Mentor' ? 'Mentee' : 'Mentor';
      // print('🔍 Current user connection preference: ${user.connectionPreference}');
      // print('🔍 Looking for users with preference: $targetRole');
      // query = query.where('Connection Preference', isEqualTo: targetRole);
      
      // TEMPORARY: Skip age filter for testing
      ...
      // TEMPORARY: Skip gender filter for testing
      // if (user.genderPreferences != 'Everyone') {
      //   query = query.where('Gender', isEqualTo: user.genderPreferences);
      // }
      
      // TEMPORARY: Skip industry filter for testing
      // if (user.matchByIndustry && user.industry.isNotEmpty) {
      //   query = query.where('Industry', isEqualTo: user.industry);
      // }
      
      // TEMPORARY: Skip excluded users filter for testing
      ...
      print('🔍 Query returned ${snapshot.docs.length} documents from Firestore');
      ...
      setState(() {
        _errorMessage = 'Error fetching mentorship connections: $e';
        _isLoading = false;
      });
    }
  }
