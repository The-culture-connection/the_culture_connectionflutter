 ...
 * Call via HTTP: https://YOUR_REGION-YOUR_PROJECT.cloudfunctions.net/generateTestUltraBlackBusinesses
 */
export const generateTestUltraBlackBusinesses = onRequest({
  cors: true,
  memory: "1GiB",
  timeoutSeconds: 540,
}, async (req: any, res: any) => {
  try {
    logger.info("🚀 Starting to generate 100 test Ultra Black Friday businesses...");
    ...
    for (let i = 0; i < 100; i++) {
      try {
        ...
        successCount++;
        if ((i + 1) % 10 === 0) {
          logger.info(`✅ Created ${i + 1}/100 businesses...`);
        }
      } catch (e: any) {
        errorCount++;
        logger.error(`❌ Error creating business ${i + 1}:`, e.message);
      }
    }
    
    logger.info(`📊 Summary: ${successCount} successful, ${errorCount} errors`);
    
    res.json({
      success: true,
      message: 'Test businesses generated successfully!',
      totalCreated: successCount,
      errors: errorCount,
    });
  } catch (error: any) {
    logger.error('❌ Failed to generate test businesses:', error);
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to generate businesses',
    });
  }
});